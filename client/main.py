...
def send_command(command):
    ...
    Функція відправляє команду у вигляді рядка, закодованого в байти,
...
def send_command1(message):
    """
    Обробляє різні команди, пов'язані з іменами та результатами.
    Функція аналізує команду, яка отримана у вигляді рядка.
    Вона обробляє команди для перевірки наявності імені, збереження
    результатів гри та отримання всіх збережених результатів.
    Команда для обробки (str).
    Відповідь про результат обробки команди.
    """
...
def custom_messagebox(title, message, style_type):
    """
    Кастомний messagebox для виведення інформації з кнопкою ОК.
    Функція створює нове вікно з заголовком та повідомленням.
    Вікно містить кнопку "OK" для закриття.
    Заголовок вікна (str).
    Повідомлення для відображення (str).
    Тип стилю для елементів (str).
    """
...
def custom_inputbox(title, message):
    """
    Кастомне вікно для введення тексту з кнопками OK і Cancel.
    Функція створює нове вікно для введення тексту,
    яке містить поле вводу та кнопки для підтвердження або скасування введення.
    Заголовок вікна (str).
    Повідомлення для відображення (str).
    Введений текст, або None, якщо вікно закрито без введення.
    """
    ...
    window.wait_window(window)  # Чекає закриття вікна
...
def save_score():
    """
    Зберігає поточний рахунок і режим у файл config.json.
    Функція запитує у користувача ім'я для збереження рахунку.
...
def load_score():
    """
    Завантажує збережені рахунки з файлу config.json.
    Функція відкриває файл конфігурації для зчитування збережених рахунків.
    ...
    def select_score():
        """
        Вибирає та завантажує вибране збереження.
        Функція отримує вибране ім'я рахунку з listbox.
    ...
    def delete_score():
        """
        Видаляє вибране збереження з файлу та listbox.
        Функція видаляє вибране ім'я рахунку з listbox
...
def reset_scores():
    ...
    Функція скидає рахунок гравців до нуля
...
def show_actions_page():
    """
    Показує сторінку з діями в грі 'Камінь-Ножиці-Папір'.
    Функція очищає вміст поточного вікна
...
def clear_window():
    ...
    Функція знищує всі віджети у вікні,
...
def start_game(mode):
    ...
    Вибраний режим гри.
    Глобальні змінні для режиму та виборів гравців
...
def show_player_choice_page(player_num):
    ...
    Номер гравця (1 або 2).
    Функція очищає вікно, відображає заголовок
...
def player_move(player_num, move):
    """
    Обробляє хід гравця.
    player_num Номер гравця (1 або 2).
    move Вибір гравця (Камінь, Ножиці, Папір).
    Зберігає вибір гравця 1 або 2 в залежності від номера
...
def ai_move():
    ...
    Функція генерує випадкові ходи для гравців
...
def ai_move_win_strategy():
    """
    AI робить хід, реагуючи на частоту виборів гравця 1.
    AI генерує вибір гравця 1, зберігає його
...
def determine_winner():
    """
    Визначає переможця гри на основі виборів гравців.
    Формує команду з виборів обох гравців,
...
def reset_scores():
    """
    Скидає рахунок перемог.
    Оновлює глобальні змінні для рахунку гравців до нуля
...
def play_again():
    """
    Починає новий раунд у поточному режимі гри.
    Вибирає функцію для початку нового раунду
...
def show_results(response):
    ...
    Очищає вікно, відображає вибір кожного гравця,
...
def resize_image(path, width, height):
    ...
    Шлях до зображення, яке потрібно змінити.
    Новий ширина зображення.
    Новий висота зображення.
    Повертає зображення в форматі PhotoImage.
    """
